...
<%
	...
	vo.setSeller_id(userinfo);
	
	try{
		int cnt = sellerInfoService.sellerUpdate(address, hp, email, zipcode, userinfo);
				
		if(cnt>0){%>
			<script type="text/javascript">
				alert("구매자 회원 수정 완료!");
location.href="<%=request.getContextPath() %>/sellerDetail.jsp?d_sellerid=<%=userinfo%>";
			</script>
		<%}else{%>
			<script type="text/javascript">
			alert("수정 실패!");
